...
> ⚠️ **측정된 결과: 에이전트는 edge 가 없다 (4회 독립 측정 일관, 확정적).** **라이브 자금 거래 금지.**
> | 측정 | 에이전트 Brier | 시장가 Brier | 승자 |
> |---|---|---|---|
> | 백테스트 Opus (5월, 컨텍스트X, n=130) | 0.2350 | 0.1637 | 시장 |
> | 백테스트 Haiku (와이드 2~6월, n=89) | 0.1911 | 0.1818 | 시장 |
> | **forward (컨텍스트O, 무편향, n=27, 6/2)** | **0.1533** | **0.0473** | **시장(~3배)** |
>
> forward 가 가장 신뢰도 높음(미래 해결분·lookahead 없음·컨텍스트 포함=실운용 형태). 컨텍스트를 켜도 시장을 못 이김.
>
> **보정 레이어 시도 결과 (`calibrate --tune`, temperature scaling, LOOCV n=27): 실패.** 원본 0.1533 → 보정 0.1483 (+0.005뿐), 시장가 0.047 와 여전히 3배 격차. 적합 T=1.70 으로 과신은 확인됐으나, 교정해도 안 됨. **진단: 문제는 confidence(과신)가 아니라 discrimination(판별력)** — LLM 확률 추정 자체가 시장만큼 정확하지 않음(방향이 틀린 예측은 보정으로 못 고침, 예: Hormuz 0.80 vs 시장 0.01→실제 0). 튜닝으로 메울 수 있는 문제가 아님.
>
> **종합 결론: 범용 LLM 으로 예측시장 베팅 = edge 없음. 라이브 자동매매 보류.** 남은 방향은 카테고리 선별(특정 분야에서만 이기는지)·전혀 다른 신호원뿐. 그때까지 dry-run/연구 전용.
