...
>
> forward 가 가장 신뢰도 높음(미래 해결분·lookahead 없음·컨텍스트 포함=실운용 형태). 컨텍스트를 켜도 시장을 못 이김. 패배 원인 = **과신**(0.6~0.7 확신이 실제 0으로 해결, 예: Strait of Hormuz 에이전트 0.80 vs 시장 0.01). edge 를 입증(에이전트 Brier < 시장가)하기 전까지 dry-run/연구 전용. 개선 시도 방향: 확률 보정(Platt/temperature) 레이어, 카테고리 선별(어디서든 이기는 분야 탐색), 또는 라이브 보류.
